...
/* Alternative implementation: each synchronized send is submitted to CXI's
 * deferred work queue and released by a remote write to its ready counter.
 * Compile this file in place of prism.c with the same includes/libraries;
 * the public API is unchanged. Use the same variant on both peers.
 *
 * PRISM_Start/Startall queue one FI_OP_WRITE per send activation. The first
 * activation still receives the peer's metadata through blocking MPI_Recv.
 * Start_no_sync/Startall_no_sync retain the direct, untriggered write path;
 * both peers must agree on whether each activation uses synchronization.
 *
 * PRISM_TRIGGER_LIMIT can lower the per-rank limit printed at initialization.
 * Keep the active send window within this limit (e.g. at least 64 credits for
 * a 64-send window). Startall can partially activate its array before returning
 * a resource error; do not blindly retry the whole array. Queued work must
 * complete through Wait/Waitall before restart, request free, or Finalize.
 * CXI errors are propagated; cancellation/recovery of failed work is not
 * implemented. The caller should terminate the benchmark on such errors.
 *
 * The service also serves other users such as MPI. The local budget cannot
 * account for their trigger usage. Leave headroom with PRISM_TRIGGER_LIMIT;
 * FI_CXI_ENABLE_TRIG_OP_LIMIT=1 enables CXI's additional service-wide check
 * at a posting-cost penalty. No provider environment settings are changed here.
 *
 * CXI 2.3.1 always uses DMA for triggered writes and its deferred-write path
 * ignores msg.desc, even with hybrid descriptors enabled. Small writes cannot
 * use FI_INJECT. Wait also reads the readiness error counter before reporting
 * success. These costs can offset the benefit of hardware overlap; performance
 * and hardware execution still need measurement on the target system.
 */
...
typedef struct mpiext_persistent_ctx {
	...
	size_t trigger_limit;
	size_t triggers_inflight;
	int progress_error;
	int n_peers;
	int my_world_rank;
...
typedef struct persistent_request {
	...
	struct fid_cntr *ready_cntr;
	/* Deferred descriptors must outlive Start and remain stable until Wait. */
	struct fi_deferred_work work;
	struct fi_op_rma triggered_write;
	struct iovec triggered_iov;
	struct fi_rma_iov triggered_rma_iov;
	void *triggered_desc;
	bool trigger_queued;
	bool op_active;
	bool synchronized_send;
	void *data_buffer;
...
mpiext_persistent_cleanup_request_ressources(persistent_request_t *req) {
	...
	if (req->ready_cntr) {
		ret = fi_close(&req->ready_cntr->fid);
		if (ret < 0) {
			OPT_PERSISTENT_ERR("ready cntr close failed");
		}
	}
...
static inline void mpiext_persistent_reset_request(
	...
	request->ready_cntr = NULL;
	memset(&request->work, 0, sizeof(request->work));
	memset(&request->triggered_write, 0, sizeof(request->triggered_write));
	request->trigger_queued = false;
	request->op_active = false;
	request->synchronized_send = false;
...
/* CXI trigger resources are shared by the service, not private to a domain.
 * Divide conservatively among all ranks on the node, and reserve one entry
 * per rank for the receive counter's fi_cntr_wait(). Other libraries (MPI)
 * may also use this service; PRISM_TRIGGER_LIMIT can leave extra headroom.
 */
static int mpiext_persistent_trigger_budget(struct fi_cxi_dom_ops *ops,
                                           size_t tx_size)
{
    MPI_Comm local_comm;
    int local_size, ret;
    size_t depth, limit;
    const char *setting = getenv("PRISM_TRIGGER_LIMIT");

    ret = ops->get_dwq_depth(&ctx.domain->fid, &depth);
    if (ret)
        return ret;
    ret = PMPI_Comm_split_type(MPI_COMM_WORLD, MPI_COMM_TYPE_SHARED, 0,
                               MPI_INFO_NULL, &local_comm);
    if (ret != MPI_SUCCESS)
        return PRISM_ERROR;
    ret = PMPI_Comm_size(local_comm, &local_size);
    PMPI_Comm_free(&local_comm);
    if (ret != MPI_SUCCESS || local_size <= 0)
        return PRISM_ERROR;

    limit = depth / (size_t)local_size;
    if (limit <= 1 || tx_size <= 1)
        return -FI_ENOSPC;
    --limit;
    /* Also leave an endpoint transmit slot for the outgoing READY writes. */
    if (limit > tx_size - 1)
        limit = tx_size - 1;
    if (limit > DEFAULT_REQUEST_STORE_SIZE)
        limit = DEFAULT_REQUEST_STORE_SIZE;
    if (setting && setting[0]) {
        char *end;
        errno = 0;
        unsigned long long requested = strtoull(setting, &end, 10);
        if (errno || setting[0] == '-' || *end || !requested || requested > limit) {
            OPT_PERSISTENT_ERR("PRISM_TRIGGER_LIMIT must be in [1, %zu]", limit);
            return -FI_EINVAL;
        }
        limit = (size_t)requested;
    }
    ctx.trigger_limit = limit;
    ctx.triggers_inflight = 0;
    ctx.progress_error = 0;
    fprintf(stderr, "PRISM triggered rank %d: limit=%zu, service depth=%zu, local ranks=%d\n",
            ctx.my_world_rank, limit, depth, local_size);
    return 0;
}
...
int PRISM_Init(void)
{
    ...
    hints->caps = FI_RMA | FI_WRITE | FI_REMOTE_WRITE | FI_RMA_EVENT | FI_TRIGGER;
    ...
    /* Enable hybrid descriptors before endpoint creation for direct writes.
     * CXI 2.3.1's FI_QUEUE_WORK RMA path still ignores the supplied descriptor.
     */
    ret = fi_open_ops(&ctx.domain->fid, FI_CXI_DOM_OPS_5, 0,
                      (void **)&cxi_ops, NULL);
    if (ret)
        goto err_cxi_setup;
    if (!cxi_ops || !cxi_ops->enable_hybrid_mr_desc || !cxi_ops->get_dwq_depth) {
        ret = -FI_ENOSYS;
        goto err_cxi_setup;
    }
    ...
    ret = mpiext_persistent_trigger_budget(cxi_ops, rma_info->tx_attr->size);
    if (ret)
        goto err_cxi_setup;
...
err_cxi_setup:
    OPT_PERSISTENT_ERR("CXI triggered setup failed: %s", fi_strerror(-ret));
    if (ctx.ep) {
        fi_close(&ctx.ep->fid);
        ctx.ep = NULL;
    }
...
static inline int mpiext_persistent_register_memory(persistent_request_t *request)
{
    ...
    ret = fi_mr_reg(ctx.domain, &request->flag_buffer, sizeof(int),
                    ...
                    receiving ? 0 : FI_RMA_EVENT, &request->flag_buffer_mr, NULL);
    if (ret)
        goto err_register;
    ...
    if (!receiving) {
        ...
        ret = fi_cntr_open(ctx.domain, &attr, &request->ready_cntr, NULL);
        if (ret)
            goto err_register;
        ret = fi_mr_bind(request->flag_buffer_mr, &request->ready_cntr->fid,
                         FI_REMOTE_WRITE);
        if (ret)
            goto err_register;
    }
...
mpiext_persistent_unpack_rdma_info(persistent_request_t *request, void *payload)
{
    ...
    /* Keep our local slot intact for freeing the MR and its trigger counter. */
    request->remote_store_index = (uint32_t)packed[4];

    if (request->op_type == PERSISTENT_SEND) {
        request->triggered_iov = (struct iovec) {
            .iov_base = request->data_buffer,
            .iov_len = request->size,
        };
        request->triggered_rma_iov = (struct fi_rma_iov) {
            .addr = 0, /* FI_MR_VIRT_ADDR is not requested: use MR offsets. */
            .len = request->size,
            .key = request->remote_data_mkey,
        };
        request->triggered_desc = fi_mr_desc(request->data_buffer_mr);
        request->triggered_write = (struct fi_op_rma) {
            .ep = ctx.ep,
            .msg = {
                .msg_iov = &request->triggered_iov,
                .desc = &request->triggered_desc,
                .iov_count = 1,
                .addr = ctx.peer_addr[request->peer_rank],
                .rma_iov = &request->triggered_rma_iov,
                .rma_iov_count = 1,
                .context = request,
            },
            /* Triggered writes cannot inject: even tiny sends need a CQE
             * before the application can reuse their source buffer.
             */
            .flags = FI_COMPLETION | FI_TRANSMIT_COMPLETE,
        };
        request->work.triggering_cntr = request->ready_cntr;
        request->work.completion_cntr = NULL; /* CXI supports CQ-only completion. */
        request->work.op_type = FI_OP_WRITE;
        request->work.op.rma = &request->triggered_write;
    }
}
...
{
    /* CXI 2.3.1 has no per-work cancellation. Never tear down memory or a
     * counter while a queued trigger can still access it.
     */
    if (!request_store)
        return PRISM_SUCCESS;
    for (uint32_t i = 0; i < DEFAULT_REQUEST_STORE_SIZE; ++i) {
        if (request_store[i].op_active)
            return -FI_EBUSY;
    }
...
mpiext_persistent_progress(int count, struct fi_cq_entry cqes[])
{
    ...
    if (rc > 0) {
        for (ssize_t i = 0; i < rc; ++i) {
                ...
                if (request->trigger_queued) {
                    request->trigger_queued = false;
                    --ctx.triggers_inflight;
                }
        }
    }

    if (PERSISTENT_UNLIKELY(rc == -FI_EAVAIL)) {
        ssize_t error_rc = fi_cq_readerr(ctx.cq, &err, 0);
        if (error_rc > 0) {
            OPT_PERSISTENT_ERR("CQ: %s", fi_cq_strerror(ctx.cq, err.prov_errno, err.err_data, NULL, 0));
        }
        ctx.progress_error = error_rc < 0 ? (int)error_rc : -FI_EIO;
        return ctx.progress_error;
    }

    if (rc < 0 && rc != -FI_EAGAIN) {
        ctx.progress_error = (int)rc;
        return ctx.progress_error;
    }
...
{
    struct fi_cq_entry cqes[MPIEXT_PERSISTENT_DEFAULT_COMP];
    int ret;

    if (request->expected_flag_ops == FI_CXI_CNTR_SUCCESS_MAX)
        return -FI_EOVERFLOW;
    if (ctx.triggers_inflight >= ctx.trigger_limit) {
        ret = mpiext_persistent_progress(MPIEXT_PERSISTENT_DEFAULT_COMP, cqes);
        if (ret)
            return ret;
        if (ctx.triggers_inflight >= ctx.trigger_limit) {
            OPT_PERSISTENT_ERR("Trigger limit %zu reached; reduce the active send window",
                               ctx.trigger_limit);
            return -FI_EAGAIN;
        }
    }

    /* Count READY writes, not the flag's value. Keeping the counter monotonic
     * handles early arrivals and prevents reuse of readiness from a prior
     * activation. No CPU polling of READY and no software counter increment.
     */
    request->work.threshold = request->expected_flag_ops + 1;
    ret = fi_control(&ctx.domain->fid, FI_QUEUE_WORK, &request->work);
    if (ret == -FI_EAGAIN || ret == -FI_ENOSPC) {
        int progress_ret = mpiext_persistent_progress(MPIEXT_PERSISTENT_DEFAULT_COMP, cqes);
        if (progress_ret)
            return progress_ret;
        /* One bounded retry. Spinning here could prevent later receives in
         * Startall from announcing readiness and deadlock the application.
         */
        ret = fi_control(&ctx.domain->fid, FI_QUEUE_WORK, &request->work);
    }
    if (ret) {
        OPT_PERSISTENT_ERR("FI_QUEUE_WORK failed: %s", fi_strerror(-ret));
        return ret;
    }
    ++request->expected_flag_ops;
    ++request->posted_ops;
    request->trigger_queued = true;
    ++ctx.triggers_inflight;
    return PRISM_SUCCESS;
}
...
static inline int mpiext_persistent_finalize_send_core(persistent_request_t *request)
{
    ...
    /* Deferred thresholds can include error events. Check the ready counter
     * before reporting success; keep this off the posting path. This performs
     * a CXI counter writeback/read once per synchronized send completion.
     */
    if (request->synchronized_send && fi_cntr_readerr(request->ready_cntr)) {
        ctx.progress_error = -FI_EIO;
        return ctx.progress_error;
    }
...
static inline int mpiext_persistent_start_recv_core(persistent_request_t *request)
{
    ...
    int ret = mpiext_persistent_start_flag_transfer(request);
    if (!ret)
        request->posted_ops++;
    return ret;
}
...
static int mpiext_persistent_activate(PRISM_Request *request, bool synchronize)
{
    int ret;
    if (!request || !*request)
        return -FI_EINVAL;
    ...
    if (ctx.progress_error)
        return ctx.progress_error;
    ret = synchronize ? mpiext_persistent_start_internal(req) :
                        mpiext_persistent_start_internal_no_sync(req);
    if (!ret) {
        req->op_active = true;
        req->synchronized_send = synchronize && req->op_type == PERSISTENT_SEND;
    }
    return ret;
}
...
{
    return mpiext_persistent_activate(request, false);
}
...
int PRISM_Startall_no_sync(int count, PRISM_Request request_array[])
{
    ...
    for (int i = 0; i < count; i++) {
        if (!request_array[i])
            continue;
        ret = PRISM_Start_no_sync(&request_array[i]);
...
{
    return mpiext_persistent_activate(request, true);
}
...
int PRISM_Startall(int count, PRISM_Request request_array[])
{
    ...
    for (int i = 0; i < count; i++) {
        if (!request_array[i])
            continue;
        ret = PRISM_Start(&request_array[i]);
...
{
    int ret;
    if (!request || !request->op_active)
        return PRISM_SUCCESS;
    if (ctx.progress_error)
        return ctx.progress_error;
    if (request->op_type == PERSISTENT_SEND)
        ret = mpiext_persistent_finalize_send_core(request);
    else
        ret = mpiext_persistent_finalize_recv_core(request);
    if (!ret)
        request->op_active = false;
    return ret;
}
...
{
    (void)status;
    return request ? mpiext_persistent_wait_internal(*request) : PRISM_SUCCESS;
}
...
int PRISM_Prequest_free(PRISM_Request *request) {
	...
	if (req->op_active || req->trigger_queued)
		return -FI_EBUSY;
