...
#define DEFAULT_REQUEST_STORE_SIZE 512
...
static inline int init_request_storage(void) {
	...
	for (int i = 0; i < DEFAULT_REQUEST_STORE_SIZE; ++i) {
...
static inline void cleanup_request_storage(void){
	...
	for(int i = 0; i < DEFAULT_REQUEST_STORE_SIZE; ++i){
		if(request_store[i].req_state == REQ_FREE)
			continue;
...
static PERSISTENT_ALWAYS_INLINE int prism_check_duplicate(uint32_t id) {
	request_entry_t *current, *tmp;
	HASH_ITER(hh, request_table, current, tmp) {
		if (current->id == id) {
			return 1;
		}
	}
...
printf("USING PRISM\n");
	...
	PMPI_Allgather(addr, addr_len, MPI_BYTE, tmp_msg_addr_buffer, addr_len,
		       MPI_BYTE, MPI_COMM_WORLD);

	for (int i = 0; i < world_size; ++i) {
		if (i == my_rank) continue;
		ret =
		    fi_av_insert(ctx.msg_av, tmp_msg_addr_buffer + i * addr_len,
				 1, &ctx.msg_peer_addr[i], 0, NULL);
		assert(ret >= 0);
	}
...
static void mpiext_persistent_cleanup_module(void) {
	...
	if (ctx.msg_av) fi_close(&ctx.msg_av->fid);
	if (ctx.msg_cq) fi_close(&ctx.msg_cq->fid);
	// if (ctx.msg_ep)
...
PERSISTENT_ALWAYS_INLINE static inline int mpiext_persistent_msg_progress(
    void) {
	...
	if (rc > 0) {
		for (ssize_t i = 0; i < rc; ++i) {
			...
			if (cqes[i].flags & FI_RECV) {
				mpiext_persistent_unpack_rdma_info(request,
								   cqes[i].buf);
...
static int mpiext_persistent_init_request(persistent_request_t *request) {
	...
	do {
		ret = fi_tsend(ctx.msg_ep, request->my_rdma_info_buffer,
			       request->my_rdma_info_size, NULL,
			       ctx.msg_peer_addr[request->peer_rank],
			       request->id, request);
		if (ret == -FI_EAGAIN) {
			mpiext_persistent_msg_progress();
		} else if (ret < 0 && ret != FI_EAGAIN) {
			OPT_PERSISTENT_ERR("fi_tsend failed with %s",
					   fi_strerror(-ret));
			return ret;
		}
	} while (ret);
	...
	if (rc > 0) {
		for (ssize_t i = 0; i < rc; ++i) {
			if (cqes[i].flags & FI_REMOTE_WRITE) {
				...
				OPT_PERSISTENT_INFO("Received message with index: %u",(uint32_t)cqes[i].data);
				request = &request_store[(uint32_t)cqes[i].data];
...
mpiext_persistent_finialize_init_first_no_sync(persistent_request_t *request) {
	...
	// request->peer_rank, request->tag,
	//           MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	// mpiext_persistent_unpack_rdma_info(request, payload);
	do {
		ret = fi_trecv(ctx.msg_ep, payload, request->my_rdma_info_size,
			       NULL, ctx.msg_peer_addr[request->peer_rank],
