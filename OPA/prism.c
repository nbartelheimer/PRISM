...
#define TO_PERSISTENT_REQUEST(_ctx_ptr) \
	container_of((_ctx_ptr), struct persistent_request, ctx)
...
int PRISM_Init(void) {
	...
	ret = fi_getinfo(FI_VERSION(2, 5), NULL, NULL, 0, hints, &ctx.fi);
	assert(ret == 0);
	fi_freeinfo(hints);
...
static inline int mpiext_persistent_register_memory(
	...
	int ret = fi_mr_reg(ctx.domain, request->data_buffer, request->size,
			    FI_WRITE | FI_REMOTE_WRITE, 0, request->store_index, 0,
			    &request->data_buffer_mr, NULL);
	if (ret < 0) {
	...
	ret = fi_mr_reg(ctx.domain, &request->flag_buffer, sizeof(int),
			FI_WRITE | FI_REMOTE_WRITE, 0, request->store_index + 512, 0,
			&request->flag_buffer_mr, NULL);
	if (ret < 0) {
	...
	if (rc > 0) {
		for (ssize_t i = 0; i < rc; ++i) {
			if (cqes[i].flags & FI_REMOTE_WRITE) {
				...
				request = TO_PERSISTENT_REQUEST(cqes[i].op_context);
				request->completed_ops++;
			}
		}
	}
